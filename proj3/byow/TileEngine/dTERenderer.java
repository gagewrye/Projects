...
public class dTERenderer {
     ...
     */
    public void renderFrame(TETile[][] world, Dungeon dungeon) {
        StdDraw.clear(new Color(0, 0, 0));
        if (dungeon.getLight()) {
            ...
            int X = dungeon.getMyPlayer().getxCoord();
            int Y = dungeon.getMyPlayer().getyCoord();
            for (int i = -5; i <= 5; i++) {
                for (int j = -5; j <= 5; j++) {
                    try {
                        world[X + i][Y + j].draw(X + i + xOffset, Y + j + yOffset);
        ...
        Font font = new Font("Monaco", Font.BOLD, TILE_SIZE - 2);
        StdDraw.setFont(font);
        StdDraw.textLeft(0.5, 0.4, dungeon.getMyPlayer().name
                + " Health: " + dungeon.getMyPlayer().getHealth()); // prints Name Health: 100
    }
}